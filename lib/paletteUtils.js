...
const PALETTE_QUERY_KEYS = ['primary', 'secondary', 'accent', 'bg', 'background', 'text'];

/** True when the URL carries shareable palette hex params (should not be indexed). */
export function hasPaletteQueryParams(searchParams) {
  if (!searchParams || typeof searchParams !== 'object') return false;

  return PALETTE_QUERY_KEYS.some((key) => {
    const raw = searchParams[key];
    if (!raw) return false;
    const value = Array.isArray(raw) ? raw[0] : raw;
    return normalizeHexParam(value) !== null;
  });
}
